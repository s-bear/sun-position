...
# Algorithm from "Solar position algorithm for solar radiation
# applications" by Ibrahim Reda and Afshin Anreas, Solar Energy (2004)
...
def julian_day(dt):
    ...
    if m <= 2:  # From paper: "if M = 1 or 2, then Y = Y - 1 and M = M + 12"
        m += 12
        y -= 1
    # day of the month with decimal time
    d = dt.day + dt.hour/24.0 + dt.minute/(24.0*60.0) + dt.second/(24.0*60.0*60.0) + dt.microsecond/(24.0*60.0*60.0*1e6)
...
def heliocentric_longitude(jme):
    ...
    #L5, ..., L0
    Li = [sum(a*np.cos(b + c*jme) for a,b,c in abcs) for abcs in reversed(_EHL_)]
...
def heliocentric_latitude(jme):
    ...
    Bi = [sum(a*np.cos(b + c*jme) for a,b,c in abcs) for abcs in reversed(_EHB_)]
...
def heliocentric_radius(jme):
    ...
    Ri = [sum(a*np.cos(b + c*jme) for a,b,c in abcs) for abcs in reversed(_EHR_)]
...
def heliocentric_position(jme):
    ...
    """
    return heliocentric_longitude(jme), heliocentric_latitude(jme), heliocentric_radius(jme)
...
def nutation_obliquity(jce):
    ...
    x0 = d2r(np.polyval([1./189474, -0.0019142, 445267.111480, 297.85036],jce))
    #mean anomaly of the sun (Earth), in radians:
    x1 = d2r(np.polyval([-1/3e5, -0.0001603, 35999.050340, 357.52772], jce))
    #mean anomaly of the moon, in radians:
    x2 = d2r(np.polyval([1./56250, 0.0086972, 477198.867398, 134.96298], jce))
    #moon's argument of latitude, in radians:
    x3 = d2r(np.polyval([1./327270, -0.0036825, 483202.017538, 93.27191], jce))
    #Longitude of the ascending node of the moon's mean orbit on the ecliptic
    # measured from the mean equinox of the date, in radians
    x4 = d2r(np.polyval([1./45e4, 0.0020708, -1934.136261, 125.04452], jce))

    x = (x0, x1, x2, x3, x4)

    dp = 0.0
    for y, ab in zip(_NLOY_, _NLOab_):
        a,b = ab
        dp += (a + b*jce)*np.sin(np.dot(x, y))
    dp = r2d(dp)/36e6

    de = 0.0
    for y, cd in zip(_NLOY_, _NLOcd_):
        c,d = cd
...
def sun_longitude(helio_pos, delta_psi):
    ...
    beta = -B
    ll = theta + delta_psi + abberation_correction(R)
    return ll, beta
...
def greenwich_sidereal_time(jd, delta_psi, epsilon):
    ...
    v = v0 + delta_psi*np.cos(d2r(epsilon))
    return v
...
def sun_ra_decl(llambda, epsilon, beta):
    ...
    l, e, b = map(d2r, (llambda, epsilon, beta))
...
def sun_topo_ra_decl_hour(latitude, longitude, elevation, jd, delta_t = 0):
    ...
    helio_pos = heliocentric_position(jme)
    R = helio_pos[-1]
    ...
    delta_psi, epsilon = nutation_obliquity(jce) #

    llambda, beta = sun_longitude(helio_pos, delta_psi) #
    
    alpha, delta = sun_ra_decl(llambda, epsilon, beta) #

    v = greenwich_sidereal_time(jd, delta_psi, epsilon) #

    H = v + longitude - alpha #
    Hr, dr = map(d2r,(H,delta))

    dar = np.arctan2(-x*np.sin(xi)*np.sin(Hr), np.cos(dr)-x*np.sin(xi)*np.cos(Hr))
    delta_alpha = r2d(dar) #
    
    alpha_prime = alpha + delta_alpha #
    delta_prime = r2d(np.arctan2((np.sin(dr) - y*np.sin(xi))*np.cos(dar), np.cos(dr) - y*np.sin(xi)*np.cos(Hr))) #
...
def sun_topo_azimuth_zenith(latitude, delta_prime, H_prime, temperature=14.6, pressure=1013):
    ...
    """
    phi = d2r(latitude)
    dr, Hr = map(d2r,(delta_prime, H_prime))
    P, T = pressure, temperature
    e0 = r2d(np.arcsin(np.sin(phi)*np.sin(dr) + np.cos(phi)*np.cos(dr)*np.cos(Hr)))
    tmp = d2r(e0 + 10.3/(e0+5.11))
    delta_e = (P/1010.0)*(283.0/(273+T))*(1.02/(60*np.tan(tmp)))
    e = e0 + delta_e
    zenith = 90 - e

    gamma = r2d(np.arctan2(np.sin(Hr), np.cos(Hr)*np.sin(phi) - np.tan(dr)*np.cos(phi))) % 360
    Phi = (gamma + 180) % 360 #azimuth from north
    return Phi, zenith


def sun_position(dt, latitude, longitude, elevation, temperature=14.6, pressure=1013, delta_t=0):
    """Compute the coordinates of the sun as viewed at the given time and location.

    Arguments:
    dt -- datetime.datetime object representing the time of observation
    ...
    Returns a tuple of (azimuth, zenith, RA, dec, H) where:
    azimuth -- in degrees, the observed azimuth angle of the sun measured eastward from north
    zenith -- in degrees, the observed elevation angle of the sun measured from vertical
    RA -- topocentric right ascension, in degrees
    dec -- topocentric declination, in degrees
    H -- topocentric hour angle, in degrees
    """

    #6367444 = radius of earth
    jd = julian_day(dt)
    RA, dec, H = sun_topo_ra_decl_hour(latitude, longitude, elevation, jd, delta_t)
    azimuth, zenith = sun_topo_azimuth_zenith(latitude, dec, H, temperature, pressure)
